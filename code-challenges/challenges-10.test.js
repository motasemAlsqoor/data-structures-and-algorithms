...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data
        .filter(x=>(x.gender=="female"||x.gender=="male")&& x.name)
        .map(x=>x.name)
        .reduce((ac,cr,i,a)=>ac+" and "+cr)
};
...
let findShortest = (data) => {
  ...
  return data
        .map(x=>x.name)
        .reduce((ac,cr)=>{
            if(ac.length<cr.length)return ac
            return cr;
        });
};
